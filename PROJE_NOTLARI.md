...
- **Bilinen kalıcı riskler (şimdilik dokunulmadı):** tekrarlayan aynı alarm metni cloud'da yayınlamayabilir (`messages` hâlâ READ_WRITE + aynı-değer bastırması); `udpReceive` sürekli `reset_upside=true` gelirse restart döngüsü riski; dolumRescue mutlak 1.0 bar eşiği / tek-loop `oto_dolum` sıfırlaması / sensör bayatken sessiz atlama.
